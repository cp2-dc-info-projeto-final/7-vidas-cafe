...
```sql
CREATE USER setevidascafe WITH ENCRYPTED PASSWORD 'cafe';
GRANT ALL ON SCHEMA public TO setevidascafe;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO setevidascafe;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO setevidascafe;
```
...
```bash
psql -h localhost  -d sete_vidas_cafe -U setevidascafe -W
```
...
```bash
 psql -h localhost  -d sete_vidas_cafe -U setevidascafe -W -f /home/labcaxias/git/7-vidas-cafe/src/api/db/create.sql
```
