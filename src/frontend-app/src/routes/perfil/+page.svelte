                          ...
                          <div class="w-full sm:w-3/4 flex flex-col sm:flex-row items-center gap-3">
                              {#if editingField === item.field}
                                  ...
                                  <div class="flex gap-2 w-full sm:w-auto shrink-0">
                                      <button on:click={() => saveField(item.field)} disabled={saveLoading} class="flex-1 sm:flex-none px-4 py-2 bg-tertiary-600 hover:bg-tertiary-500 text-primary-950 font-bold rounded-lg text-[10px] uppercase tracking-wider transition-all shadow-md cursor-pointer">Salvar</button>
                                      <button on:click={cancelEdit} disabled={saveLoading} class="flex-1 sm:flex-none px-4 py-2 bg-primary-800 hover:bg-primary-700 text-primary-100 font-bold rounded-lg text-[10px] uppercase tracking-wider transition-all cursor-pointer">Sair</button>
                                  </div>
                              {:else}
                                  <div class="w-full text-primary-100 text-xs font-medium truncate flex items-center py-1">
                                      {item.val}
                                  </div>
                                  <button on:click={() => startEdit(item.field, item.field === 'dat_nas' ? user.dat_nas : (item.field === 'password' ? '' : item.val))} class="w-full sm:w-auto shrink-0 px-4 py-2 bg-primary-900 hover:bg-primary-800 border border-primary-700 text-tertiary-300 hover:text-tertiary-200 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer">Editar</button>
                              {/if}
                          </div>
                      </div>
                  {/each}
              ...
              <div class="mt-4 pt-6 border-t border-primary-800 bg-red-950/20 p-5 rounded-xl border border-red-900/30 flex flex-col gap-4">
                  <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                      ...
                      {#if !showDeleteConfirmation}
                          <button on:click={() => { showDeleteConfirmation = true; error = ''; }} class="w-full md:w-auto shrink-0 px-4 py-2.5 bg-red-950/60 hover:bg-red-900/50 border border-red-800 text-red-300 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all shadow-sm cursor-pointer">Excluir conta</button>
                      {/if}
                  </div>
                  ...
                  {#if showDeleteConfirmation}
                      <div class="mt-2 p-4 bg-primary-950 border border-red-900/50 rounded-xl flex flex-col gap-3 shadow-inner">
                          ...
                          <div class="flex flex-col sm:flex-row gap-2">
                              ...
                              <div class="flex gap-2 w-full sm:w-auto shrink-0">
                                  <button type="button" on:click={deleteOwnAccount} disabled={saveLoading || !confirmPassword} class="flex-1 sm:flex-none px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors disabled:opacity-50 cursor-pointer">{saveLoading ? 'Excluindo...' : 'Confirmar'}</button>
                                  <button type="button" on:click={() => { showDeleteConfirmation = false; confirmPassword = ''; }} disabled={saveLoading} class="flex-1 sm:flex-none px-4 py-2.5 bg-primary-800 hover:bg-primary-700 text-primary-100 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors cursor-pointer">Cancelar</button>
                              </div>
                          </div>
                      </div>
                  {/if}
      ...
      <div class="w-full max-w-md bg-primary-900 border border-tertiary-600/60 p-6 sm:p-8 rounded-2xl shadow-2xl flex flex-col gap-5">
          ...
          <div class="flex gap-3 justify-end mt-2">
              <button 
                  ...
                  class="px-4 py-2.5 bg-primary-800 hover:bg-primary-700 text-primary-100 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors cursor-pointer">
                  Cancelar
              </button>
              <button 
                  type="button" 
                  ...
                  disabled={saveLoading} 
                  class="px-4 py-2.5 bg-tertiary-600 hover:bg-tertiary-500 text-primary-950 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-colors shadow-md cursor-pointer">
